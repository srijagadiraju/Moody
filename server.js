...
const corsOptions = {
  ...
  methods: ["GET", "POST", "PUT", "OPTIONS"], // Added OPTIONS for preflight requests
...
async function main() {
  try {
    ...
    // Middleware to check if a user is authenticated
    function ensureAuthenticated(req, res, next) {
      console.log("User session in ensureAuthenticated:", req.user); // Log session
      if (req.isAuthenticated()) {
        return next();
      }
      res.status(401).json({ message: "Unauthorized" });
    ...
    app.post("/api/posts", ensureAuthenticated, async (req, res) => {
      console.log("User session in /api/posts:", req.user);
      try {
        const { subject, message } = req.body;
        if (!subject || !message) {
          return res
    ...
    app.put("/api/posts/:id/like", ensureAuthenticated, async (req, res) => {
      console.log("User session in /api/posts/:id/like:", req.user);
      try {
        const postId = req.params.id;
        const result = await postsCollection.updateOne(
          { _id: new ObjectId(postId) },
    ...
    app.post(
      ...
      ensureAuthenticated,
      async (req, res) => {
        console.log("User session in /api/posts/:id/comment:", req.user);
        try {
          const postId = req.params.id;
          const { text } = req.body;
